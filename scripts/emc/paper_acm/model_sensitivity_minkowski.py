...
observable = emc.MinkowskiFunctionals(
    select_mocks={
...
for param_name in ['w0_fld', 'wa_fld']:
    ...
    for i, param_value in enumerate(prior_sample):
        ...
        ax.plot(pred_y[:int(len(s)/3)], color=cmap(prior_norm[i]))
    ...
    ax.set_ylabel(r'$\textrm{Minkowski Functionals }W_i$', fontsize=13)
    ax.set_xlabel(r'$\textrm{bin index}$', fontsize=13)
    plt.tight_layout()
    plt.subplots_adjust(hspace=0.0)

    plt.savefig(f'model_sensitivity_minkowski_{param_name}.png', dpi=300, bbox_inches='tight')